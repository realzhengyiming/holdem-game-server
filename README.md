...
没有配置 `SMTP_HOST` 时，服务器会进入本地验证码模式：验证码不会真的发送邮件，会打印到服务端日志，也会直接显示在网页上，方便先跑通注册和登录。正式公网开放时建议配置 SMTP。
...
## Godot + FastAPI 新端（兼容迁移）

旧网页客户端 `public/` 保持不变。新的独立前端位于 [`godot-client`](godot-client)，可用 Godot 4 导出为 Web、桌面或移动端；新的服务边界位于 [`fastapi-gateway`](fastapi-gateway)。

FastAPI 目前作为兼容网关，Godot 只访问它，现有 Node 服务继续作为已经回归验证过的权威牌局引擎。因此新的 UI 可以立即保留邮箱验证码、房间、实时同步、断线恢复、边池结算、公平验证等功能。详细启动命令见两个目录各自的 README。
